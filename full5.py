...
LEFT_INDEX = 12
RIGHT_INDEX = 13
CODE_INDEX = 7   # <-- Important for FAR logic sync
...
with mss.mss() as sct:
    ...
    while True:
        ...
        left_state = None
        right_state = None
        code_game_state = None

        # First pass: detect everything
        for x1, y1, w, h, label, index in objects:
            ...
            if index == LEFT_INDEX:
                ...
                code_game_state = game_state

        # Send CODE state to Arduino (for FAR logic)
        if code_game_state is not None:
            send_code_state(code_game_state)
